...
export function buildApprovalRequestEmail(data: ApprovalRequestData): string {
  ...
  const money = (n: number) => formatCurrency(n, cur, numLocale);
  const t = lang === "en"
    ? { subtitle: "Approval required", greeting: "Hi", intro: "Your approval is required for the following quote:", quote: "Quote", client: "Client", total: "Total", date: "Date", moreItems: (n: number) => `...and ${n} more items`, reason: "Reason:", colDesc: "Description", colQty: "Qty", approve: "Approve", reject: "Reject", reviewPre: "Or review the full details: ", reviewLink: "view quote" }
    : { subtitle: "Aprobación requerida", greeting: "Hola", intro: "Se requiere tu aprobación para la siguiente cotización:", quote: "Cotización", client: "Cliente", total: "Total", date: "Fecha", moreItems: (n: number) => `...y ${n} items más`, reason: "Motivo:", colDesc: "Descripción", colQty: "Cant.", approve: "Aprobar", reject: "Rechazar", reviewPre: "O revisa los detalles completos: ", reviewLink: "ver cotización" };
  ...
  const moreItems =
    data.lineItems.length > 5
      ? `<tr><td colspan="3" style="padding:6px 8px;color:#888;font-style:italic;">${t.moreItems(data.lineItems.length - 5)}</td></tr>`
      : "";
  ...
  <div style="max-width:600px;margin:0 auto;padding:20px;">
    ...
    <div style="background:white;padding:30px;border-radius:0 0 12px 12px;border:1px solid #e5e5e5;border-top:none;">
      ...
      ${
        ...
        <thead><tr style="background:#f8f9fa;"><th style="padding:8px;text-align:left;font-size:11px;text-transform:uppercase;color:#888;">${t.colDesc}</th><th style="padding:8px;text-align:right;font-size:11px;text-transform:uppercase;color:#888;">${t.colQty}</th><th style="padding:8px;text-align:right;font-size:11px;text-transform:uppercase;color:#888;">${t.total}</th></tr></thead>
      ...
      <div style="text-align:center;margin:24px 0;">
        <a href="${approveUrl}" style="display:inline-block;padding:12px 32px;background:#16a34a;color:white;text-decoration:none;border-radius:8px;font-weight:bold;font-size:14px;margin:0 8px;">${t.approve}</a>
        <a href="${rejectUrl}" style="display:inline-block;padding:12px 32px;background:#dc2626;color:white;text-decoration:none;border-radius:8px;font-weight:bold;font-size:14px;margin:0 8px;">${t.reject}</a>
      </div>
...
export function buildApprovalResolvedEmail(data: ApprovalResolvedData): string {
  ...
  const t = lang === "en"
    ? { subtitle: "Approval resolved", status: isApproved ? "Approved" : "Rejected", statusLower: isApproved ? "approved" : "rejected", quotePre: "Quote", forWord: "for", byWord: "by", comment: "Comment:", viewQuote: "View quote" }
    : { subtitle: "Aprobación resuelta", status: isApproved ? "Aprobada" : "Rechazada", statusLower: isApproved ? "aprobada" : "rechazada", quotePre: "La cotización", forWord: "para", byWord: "por", comment: "Comentario:", viewQuote: "Ver cotización" };
