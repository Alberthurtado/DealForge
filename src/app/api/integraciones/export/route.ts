...
export async function GET(request: NextRequest) {
  ...
  if (tipo === "clientes") {
    ...
    data = clientes.map((c) => ({
      ...
      cif: c.ruc || "",
      notas: c.notas || "",
      contacto_principal_nombre: c.contactos[0]?.nombre || "",
      contacto_principal_email: c.contactos[0]?.email || "",
      contacto_principal_cargo: c.contactos[0]?.cargo || "",
      contacto_principal_telefono: c.contactos[0]?.telefono || "",
    }));
    ...
    data = cotizaciones.map((c) => ({
      ...
      subtotal: c.subtotal,
      descuento_global: c.descuentoGlobal,
      impuesto: c.impuesto,
      total: c.total,
      moneda: c.moneda,
