...
const STR = {
  es: {
    backToQuote: "Volver a cotización", expiredBadge: "Cotización vencida", print: "Imprimir / PDF",
    quoteTitle: "COTIZACIÓN", issued: "Emisión:", validity: "Validez:", validUntil: "Válida hasta:",
    expiredParen: " (VENCIDA)", client: "Cliente", contact: "Contacto",
    thDescription: "Descripción", thQty: "Cant.", thUnitPrice: "Precio Unit.", thDiscount: "Dto.", thTotal: "Total",
    subtotal: "Subtotal", discount: "Descuento", tax: "IVA", taxNotIncluded: "IVA no incluido", total: "Total",
    notes: "Notas", terms: "Términos y Condiciones", clientSignature: "Firma del Cliente", signatureAlt: "Firma",
    signedOn: "Firmado el", generatedBy: "Cotización generada por", footerIssued: "Emitida:",
  },
  en: {
    backToQuote: "Back to quote", expiredBadge: "Quote expired", print: "Print / PDF",
    quoteTitle: "QUOTE", issued: "Issued:", validity: "Valid until:", validUntil: "Valid until:",
    expiredParen: " (EXPIRED)", client: "Client", contact: "Contact",
    thDescription: "Description", thQty: "Qty", thUnitPrice: "Unit Price", thDiscount: "Disc.", thTotal: "Total",
    subtotal: "Subtotal", discount: "Discount", tax: "Tax", taxNotIncluded: "Tax not included", total: "Total",
    notes: "Notes", terms: "Terms & Conditions", clientSignature: "Client Signature", signatureAlt: "Signature",
    signedOn: "Signed on", generatedBy: "Quote generated by", footerIssued: "Issued:",
  },
};
...
export default function CotizacionPreviewPage() {
  ...
  const money = (n: number) => formatCurrency(n, cotizacion.moneda || "EUR", numLocale);
        ...
        <div className="bg-white shadow-lg rounded-lg print:shadow-none print:rounded-none">
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  <h2 className="text-2xl font-bold tracking-tight">{t.quoteTitle}</h2>
                  <p className="text-lg font-semibold mt-1">{cotizacion.numero}</p>
                  <div className="mt-2 space-y-0.5">
                    <p className="text-xs text-white/70">
                      {t.issued} {formatDate(cotizacion.fechaEmision)}
                    </p>
                    {cotizacion.fechaVencimiento && (
                      <p className={`text-xs ${isExpired ? "text-red-200 font-medium" : "text-white/70"}`}>
                        {t.validity} {formatDate(cotizacion.fechaVencimiento)}
                        {isExpired && t.expiredParen}
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  <div className="mt-2 space-y-0.5">
                    ...
                    {cotizacion.fechaVencimiento && (
                      <p className={`text-xs ${isExpired ? "text-red-500" : "text-gray-400"}`}>
                        {t.validUntil} {formatDate(cotizacion.fechaVencimiento)}
                        {isExpired && t.expiredParen}
                      </p>
                    )}
                  </div>
              ...
              <div className="flex items-end gap-6">
                <div>
                  ...
                  <p className="text-[10px] text-gray-400">
                    {t.signedOn} {formatDate(signatureData.signedAt)}
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-[10px] text-gray-400">
                {t.generatedBy} {empresa.nombre}
                {empresa.web ? ` \u2022 ${empresa.web}` : ""}
              </p>
              <p className="text-[10px] text-gray-400">
                {cotizacion.fechaVencimiento
                  ? `${t.validUntil} ${formatDate(cotizacion.fechaVencimiento)}`
                  : `${t.footerIssued} ${formatDate(cotizacion.fechaEmision)}`}
              </p>
            </div>
          </div>
        </div>
